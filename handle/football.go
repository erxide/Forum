...
// Football gere l'affichage sur la page d'acceuil avec les posts dont le theme est Football
func Football(w http.ResponseWriter, r *http.Request) {
	...
	// recuperation des postes  avec le theme football depuis la base de donnée via une requette sql
	POSTES, err := forum.Bd.Query("SELECT id, theme, titre, description, cree_le, cree_par, likes, dislikes FROM Postes WHERE theme = 'football' ORDER BY id DESC;")
	defer POSTES.Close()
	// gestion de l'erreur
	if err != nil {
