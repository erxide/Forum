...
// MesPoste gere l'affichage sur la page d'acceuil que des postes de l'utilisateur
func MesPostes(w http.ResponseWriter, r *http.Request) {
	...
	pageconnecte := template.Must(template.ParseFiles("./templates/mespostes.html"))
	...
	POSTES, err := forum.Bd.Query("SELECT id, theme, titre, description, cree_le, cree_par, likes, dislikes FROM Postes WHERE cree_par = ? ORDER BY id DESC;", pseudo)
	defer POSTES.Close()
	// gestion de l'erreur
	if err != nil {
